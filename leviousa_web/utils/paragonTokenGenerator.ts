...
export async function generateParagonToken(userId?: string): Promise<string> {
  ...
  // Always use the API route for proper JWT signing with RSA key
  // This works in both development and production since Vercel has the environment variables
  try {
    ...
    // Cache the token (expires in 23 hours, accounting for 24-hour server token)
