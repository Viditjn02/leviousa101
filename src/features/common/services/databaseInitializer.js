...
class DatabaseInitializer {
    constructor() {
        ...
        //   macOS: ~/Library/Application Support/Leviousa/leviousa.db
//   Windows: %APPDATA%\Leviousa\leviousa.db
this.dbPath = path.join(userDataPath, 'leviousa.db');
        this.dataDir = userDataPath;

        // The original DB path (read-only location in the package)
        this.sourceDbPath = app.isPackaged
            ? path.join(process.resourcesPath, 'data', 'leviousa.db')
: path.join(app.getAppPath(), 'data', 'leviousa.db');
    }
    ...
    async createNewDatabase() {
        ...
        try {
            ...
            // No longer check for default user - Firebase authentication only
            console.log('[DatabaseInitializer] Database created successfully (Firebase authentication only)');
            return { success: true };
    ...
    async connectToExistingDatabase() {
        ...
        try {
            ...
            // No longer check for default user - Firebase authentication only
            console.log('[DatabaseInitializer] Connection to existing DB successful (Firebase authentication only)');
            return { success: true };
    ...
    async validateAndRecoverData() {
        ...
        try {
            ...
            // We just need to ensure default preset templates are present.
            await sqliteClient.synchronizeSchema();

            // Check if default preset templates exist (global templates with uid = NULL)
            const presetTemplates = await sqliteClient.getDb().prepare(
                'SELECT COUNT(*) as count FROM prompt_presets WHERE is_default = 1'
            ).get();
            
            if (!presetTemplates || presetTemplates.count === 0) {
