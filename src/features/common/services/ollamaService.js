...
class OllamaService extends EventEmitter {
    ...
    async makeRequest(endpoint, options = {}) {
        ...
        this.activeRequest = axios(`${this.baseUrl}${endpoint}`, {
            ...options,
    ...
    async healthCheck() {
        try {
            ...
            try {
                ...
                checks.serviceRunning = psResponse.status === 200;
                checks.memoryStatus = psResponse.status === 200;
            } catch (error) {
            ...
            try {
                ...
                checks.apiResponsive = rootResponse.status === 200;
            } catch (error) {
            ...
            try {
                ...
                checks.modelsAccessible = tagsResponse.status === 200;
            } catch (error) {
    ...
    async downloadFile(url, destination, options = {}) {
        const { 
            onProgress = null,
                            headers = { 'User-Agent': 'Leviousa-App' },
            timeout = 300000,
            modelId = null
        } = options;
