...
class ModelStateService extends EventEmitter {
    ...
    async setFirebaseVirtualKey(virtualKey) {
        ...
        await this.setApiKey('openai-leviousa', virtualKey);

        if (virtualKey) {
            // 이전에 설정된 적이 없는 경우 (최초 로그인)에만 모델을 강제로 변경합니다.
            if (!wasPreviouslyConfigured) {
                            console.log('[ModelStateService] First-time setup for openai-leviousa, setting default models.');
            const llmModel = PROVIDERS['openai-leviousa']?.llmModels[0];
            const sttModel = PROVIDERS['openai-leviousa']?.sttModels[0];
                if (llmModel) await this.setSelectedModel('llm', llmModel.id);
                if (sttModel) await this.setSelectedModel('stt', sttModel.id);
            } else {
                console.log('[ModelStateService] openai-leviousa key updated, but respecting user\'s existing model selection.');
            }
        } else {
            // 로그아웃 시, 현재 활성화된 모델이 openai-leviousa인 경우에만 다른 모델로 전환합니다.
            ...
            if (llmProvider === 'openai-leviousa') typesToReselect.push('llm');
            if (sttProvider === 'openai-leviousa') typesToReselect.push('stt');
    ...
    async setApiKey(provider, key) {
        ...
        // 'openai-leviousa'는 자체 인증 키를 사용하므로 유효성 검사를 건너뜁니다.
        if (provider !== 'openai-leviousa') {
