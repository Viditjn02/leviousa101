    ...
    async handleScreenClick() {
        ...
        try {
            ...
            const userConfirmed = confirm(
                'System Preferences will open to the Screen Recording settings.\n\n' +
                'In the Privacy & Security settings:\n' +
                '1. Look for "Leviousa" or "Electron" in the app list\n' +
                '2. Check the checkbox next to it\n' +
                '3. Return to this app\n\n' +
                'Click OK to continue to System Preferences.'
            );
            
            if (userConfirmed) {
                await window.api.permissionHeader.openSystemPreferences('screen-recording');
                
                // Check permissions again after user returns (with polling)
                console.log('[PermissionHeader] Starting permission check polling...');
                let checkCount = 0;
                const maxChecks = 30; // Poll for up to 60 seconds
                
                const pollPermissions = setInterval(async () => {
                    checkCount++;
                    try {
                        const updatedPermissions = await window.api.permissionHeader.checkSystemPermissions();
                        if (updatedPermissions.screen === 'granted') {
                            console.log('[PermissionHeader] Screen recording permission granted!');
                            this.screenGranted = 'granted';
                            this.requestUpdate();
                            clearInterval(pollPermissions);
                        } else if (checkCount >= maxChecks) {
                            console.log('[PermissionHeader] Permission polling timeout - stopping checks');
                            clearInterval(pollPermissions);
                        }
                    } catch (error) {
