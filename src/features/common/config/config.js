...
class Config {
    constructor() {
        ...
        this.defaults = {
            ...
            webUrl: process.env.leviousa_WEB_URL || 'https://www.leviousa.com',
    ...
    loadEnvironmentConfig() {
        if (process.env.leviousa_API_URL) {
            this.config.apiUrl = process.env.leviousa_API_URL;
            console.log(`[Config] API URL from env: ${this.config.apiUrl}`);
        }
        
        if (process.env.leviousa_WEB_URL) {
            this.config.webUrl = process.env.leviousa_WEB_URL;
            console.log(`[Config] Web URL from env: ${this.config.webUrl}`);
        }
        
        if (process.env.leviousa_API_TIMEOUT) {
            this.config.apiTimeout = parseInt(process.env.leviousa_API_TIMEOUT);
        }
        
        if (process.env.leviousa_ENABLE_JWT) {
            this.config.enableJWT = process.env.leviousa_ENABLE_JWT === 'true';
        }
        
        if (process.env.leviousa_CACHE_TIMEOUT) {
            this.config.cacheTimeout = parseInt(process.env.leviousa_CACHE_TIMEOUT);
        }
        
        if (process.env.leviousa_LOG_LEVEL) {
            this.config.logLevel = process.env.leviousa_LOG_LEVEL;
        }
        
        if (process.env.leviousa_DEBUG) {
            this.config.enableDebugLogging = process.env.leviousa_DEBUG === 'true';
        }
    ...
    getUserConfigPath() {
        const configDir = path.join(os.homedir(), '.leviousa');
